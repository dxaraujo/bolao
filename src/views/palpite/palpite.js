																		...
																		return (
																			<div key={idx3 + '-' + palpite.placarTimeA + '-' + palpite.placarTimeB} className='bg-gray-100 rodada p-2'>
																				...
																				<div className='palpiteTimeA'>
																					<input name='placarTimeA' type='text' className='palpiteTimeA form-control' maxLength='1' disabled={this.props.fase.status === 'B' || moment(palpite.partida.data, 'YYYY/MM/DD hh:mm:ss').isBefore(new Date())} tabIndex={tabIndex++} ref={input => { this.inputTabIndex[inputIndex++] = input }} value={palpite.placarTimeA} onKeyDown={e => this.handleKeyDown(e, palpite)} onChange={e => this.handleChange(e, palpite)} />
																				</div>
																				<div className='divisorPalpite'>x</div>
																				<div className='palpiteTimeB'>
																					<input name='placarTimeB' type='text' className='palpiteTimeB form-control' maxLength='1' disabled={this.props.fase.status === 'B' || moment(palpite.partida.data, 'YYYY/MM/DD hh:mm:ss').isBefore(new Date())} tabIndex={tabIndex++} ref={input => { this.inputTabIndex[inputIndex++] = input }} value={palpite.placarTimeB} onKeyDown={e => this.handleKeyDown(e, palpite)} onChange={e => this.handleChange(e, palpite)} />
																				</div>
